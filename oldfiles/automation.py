...
def generate_lab_config_with_routers(num_clusters, min_machines, max_machines, client_image, router_image):
	...
	# Define router device with its image AND privileged fla
	config_lines.append(f"{router_name}[image]={router_image}    $") # Original image line
	config_lines.append(f"{router_name}[privileged]=true    $") # ADD THIS LINE - tells Kathara to run container privileged
