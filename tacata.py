...
def parse():
    ...
    # Inside the parse function in tacata.py

    # Add 'import re' at the top of tacata.py if it's not already there
    # Add 'import traceback' if using the testing flag logic at the end

    # ... (try/except block starts) ...
    try:
        currLab = Lab()
        with open("../lab.confu", "r") as labfile:
            log("- `lab.confu` file found in specified folder, starting dump...")
            currentLine = 0  # Initialize line counter

            for line in labfile:
                currentLine += 1  # Increment line counter
                ...
                if "$" not in line:
                    print(f"Warning: Skipping line {currentLine} because it's missing the '$' separator: {line}")
                    continue
                netkitDef, commands = line.split("$", 1)
                netkitDef = netkitDef.strip()
                commands = commands.strip()

                # Reset context for each line.
                currDevice, currInterface, currLan = None, None, None

                if netkitDef != "":
                    currLab.addConfLine(netkitDef)

                    is_interface_def = re.search(r"\[\d+\]=", netkitDef)

                    if is_interface_def:
                        # --- Processing Interface Definition Line ---
                        try:
                            currDeviceName, currInterfaceNum, currLanName = parseDeviceAndInterface(netkitDef)

                            # 1. Get/Create Device and Lan objects
                            currDevice = currLab.getOrNew(currDeviceName)
                            currLan = currLab.getOrNewLan(currLanName)

                            # 2. Create Interface object and add to device
                            currInterface = Interface(currDevice, currInterfaceNum)
                            currDevice.services.append(currInterface)

                            # --- MODIFICATION START ---
                            # 3. Process commands NOW - This should set the IP on currInterface via _ip handler
                            if commands:
                                parseCommands(commands, currDevice=currDevice, currInterface=currInterface,
                                              currLab=currLab)

                            # 4. Add interface to LAN ONLY if IP was successfully set
                            if currInterface.ip is not None:
                                currLan.addInterface(currInterface)  # Call this AFTER commands are parsed
                            else:
                                # Optional: Warn if an interface is defined but has no IP command or IP is invalid
                                print(
                                    f"Warning: Line {currentLine}: Interface {netkitDef} defined but IP address was not set via commands.")
                            # --- MODIFICATION END ---

                        except Exception as e:
                            # Catch errors during interface processing specifically
                            print(
                                f"Error at line {currentLine}: Failed processing interface definition '{netkitDef}'. Details: {e}")
                            raise  # Re-raise exception to stop the script

                    else:
                        # --- Processing Non-Interface Definition Line (e.g., image) ---
                        match_dev_name = re.match(r"([^\[]+)\[", netkitDef)
                        if match_dev_name:
                            currDeviceName = match_dev_name.group(1)
                            currDevice = currLab.getOrNew(currDeviceName)  # Get device context

                        # Process commands for this line (currInterface is None)
                        if commands:
                            parseCommands(commands, currDevice=currDevice, currInterface=None, currLab=currLab)

        # --- End of loop ---
        currLab.dump()  # Dump after processing all lines

    except Exception as e:
        # General error catching including the specific line number
        print(f"Error at line {currentLine}: {str(e)}")
        if args.testing:  # Assuming args is defined via argparse
            traceback.print_exc(e)  # Print full traceback if testing flag is set
        exit()

    # Make sure 'traceback' is imported at the top if using testing flag
